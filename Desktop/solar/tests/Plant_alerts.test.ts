// CRUD
// tests/Plant_alerts.test.ts

import request from 'supertest';
import express, { Express } from 'express';
import bodyParser from 'body-parser';
import Plant_alertsRoute from '../src/routes/Plant_alertsRoute';
import { Plant_alertsService } from '../src/services/Plant_alertsService';
import { verifyToken } from '../src/middlewares/auth';

jest.mock('../src/services/Plant_alertsService');  // Mock the service
...
app.use('/solar', Plant_alertsRoute);
...
const columnsOfTable = [
  { columnName: 'alert_id', columnType: 'INT' },
...
describe('Plant_alertsController', () => {
  ...
  it('should create a new entity and return 201 status', async () => {
    ...
    (Plant_alertsService.createPlant_alerts as jest.Mock).mockResolvedValue(mockEntity);

    const response = await request(app)
      .post('/solar/Plant_alerts')
      .set('Authorization', 'Bearer mockToken')
      .send(mockEntity);

    expect(response.status).toBe(201);
    expect(response.body).toEqual({
      message: 'Plant_alerts created successfully',
      record: mockEntity,
    });
  ...
  it('should retrieve all entities and return 200 status', async () => {
    (Plant_alertsService.getAllPlant_alertss as jest.Mock).mockResolvedValue(mockEntities);

    const response = await request(app)
      .get('/solar/Plant_alerts')
      .set('Authorization', 'Bearer mockToken');
  ...
  it('should retrieve an entity by ID and return 200 status', async () => {
    const mockEntity = generateMockData(columnsOfTable);
    (Plant_alertsService.getPlant_alertsById as jest.Mock).mockResolvedValue(mockEntity);

    const response = await request(app)
      .get('/solar/Plant_alerts/1')
      .set('Authorization', 'Bearer mockToken');
  ...
  it('should return 404 if entity is not found', async () => {
    (Plant_alertsService.getPlant_alertsById as jest.Mock).mockResolvedValue(null);

    const response = await request(app)
      .get('/solar/Plant_alerts/999')
      .set('Authorization', 'Bearer mockToken');

    expect(response.status).toBe(404);
    expect(response.body).toEqual({ message: 'Plant_alerts not found' });
    expect(verifyToken).toHaveBeenCalledWith('Bearer mockToken');
  });
  ...
  it('should update an entity and return 200 status', async () => {
    ...
    (Plant_alertsService.updatePlant_alerts as jest.Mock).mockResolvedValue(updatedEntity);

    const response = await request(app)
      .put('/solar/Plant_alerts/1')
      .set('Authorization', 'Bearer mockToken')
      .send(updatedEntity);

    expect(response.status).toBe(200);
    expect(response.body).toEqual({
      message: 'Plant_alerts updated successfully',
  ...
  it('should return 404 if entity to update is not found', async () => {
    const updatedEntity = generateMockData(columnsOfTable);
    (Plant_alertsService.updatePlant_alerts as jest.Mock).mockResolvedValue(null);

    const response = await request(app)
      .put('/solar/Plant_alerts/999')
      .set('Authorization', 'Bearer mockToken')
      .send(updatedEntity);
  ...
  it('should delete an entity and return 200 status', async () => {
    (Plant_alertsService.deletePlant_alerts as jest.Mock).mockResolvedValue(true);

    const response = await request(app)
      .delete('/solar/Plant_alerts/1')
      .set('Authorization', 'Bearer mockToken');

    expect(response.status).toBe(200);
    expect(response.body).toEqual({ message: 'Plant_alerts deleted successfully' });
    expect(verifyToken).toHaveBeenCalledWith('Bearer mockToken');
  });

  it('should return 404 if entity to delete is not found', async () => {
    (Plant_alertsService.deletePlant_alerts as jest.Mock).mockResolvedValue(null);

    const response = await request(app)
      .delete('/solar/Plant_alerts/999')
      .set('Authorization', 'Bearer mockToken');
